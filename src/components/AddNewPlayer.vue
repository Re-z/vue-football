<template>
  <form
    class="new-player-form md-layout md-gutter"
  ...
  >
    ...
    <div class="md-layout-item">
      ...
      <md-field :class="{ 'md-invalid': errors.rating }">
        <label>Rating</label>
        <md-input
          v-model.lazy="newPlayer.rating"
          type="number"
          max="10"
          @blur="validateRatingField()"
        ></md-input>
      ...
      <md-field :class="{ 'md-invalid': errors.aboutInfo }">
        ...
        <md-textarea
          md-autogrow
          v-model="newPlayer.aboutInfo"
          @blur="validateAboutInfoField()"
        ></md-textarea>
        <span class="md-error">{{ errors.aboutInfo }}</span>
      </md-field>
      <md-button type="submit" class="md-raised md-accent new-player-form__btn"
        >Add new player to database</md-button
      >
    </div>
  </form>
</template>
  ...
  methods: {
    addNewPlayer() {
      if (
        ...
        this.validateRatingField() &&
        this.validateAboutInfoField()
      ) {
    ...
    validateRatingField() {
      if (
        this.newPlayer.rating.length > 2 ||
        this.newPlayer.rating.length === 0
      ) {
